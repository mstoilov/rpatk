...
int main(int argc, char *argv[])
{
	...
	vmcode[off++] = rvm_asm(RVM_LDR, R0, DA, XX, &s);
	VMTEST_REG(vmcode, off, 0, -2, "LDR");
	vmcode[off++] = rvm_asm(RVM_LDRB, R0, DA, XX, &s);
	VMTEST_REG(vmcode, off, 0, 0xFE, "LDRB");
	vmcode[off++] = rvm_asm(RVM_LDRH, R0, DA, XX, &s);
	VMTEST_REG(vmcode, off, 0, 0xFFFE, "LDRH");
	
	vmcode[off++] = rvm_asm(RVM_MOV, R5, DA, XX, &d);	
