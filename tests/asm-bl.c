...
int main(int argc, char *argv[])
{
	ruint ret = 0;
	ruint off = 0;
	ruint l_add2 = 0, l_add3 = 0, l_main = 0;
	...
	vmcode[off]   = rvm_asmi(RVM_BL,   DA, XX, XX, l_add2 - off); off++;
	...
	vmcode[off]   = rvm_asmi(RVM_BL,  DA, XX, XX, l_add3 - off); off++;
	VMTEST_REG(vmcode, off, 0, 7, "BL/RET");
	vmcode[off++] = rvm_asmi(RVM_EXT, R0, XX, XX, 0);
	vmcode[off]   = rvm_asmi(RVM_B,   DA, XX, XX, l_main - off); off++;
#ifdef EXECDEBUG
	ret = rvm_cpu_exec_debug(vm, vmcode, off - 1);
#else
	ret = rvm_cpu_exec(vm, vmcode, off - 1);
#endif
	rvm_cpu_destroy(vm);
	return 0;
}
