...
static void rvm_swi_eadd(rvmcpu_t *cpu, rvm_asmins_t *ins)
{
	...
	rvm_opmap_invoke_binary_handler(cpu->opmap, RVM_OPID_ADD, cpu, res, arg2, res);
...
int print_callback(const char *name, void *userdata, const char *input, unsigned int size, unsigned int reason, const char *start, const char *end)
{
#ifdef DEBUGPRINT
	fprintf(stdout, "%s: ", name);
//	fwrite(input, sizeof(char), size, stdout);
...
int codegen_opcode_unary_callback(const char *name, void *userdata, const char *input, unsigned int size, unsigned int reason, const char *start, const char *end)
{
	...
	if (r_strncmp(input, "-", size) == 0)
		r_array_push(co->opcodes, RVM_OPSWI(rvm_cpu_getswi(co->cpu, "RVM_SWI_NEG")), ruint);
	else
...
int codegen_unary_asmop_callback(const char *name, void *userdata, const char *input, unsigned int size, unsigned int reason, const char *start, const char *end)
{
	...
	rvm_codegen_addins(co->cg, rvm_asm(RVM_POP, R1, XX, XX, 0));
	rvm_codegen_addins(co->cg, rvm_asm(opcode, R0, R1, XX, 0));
	rvm_codegen_addins(co->cg, rvm_asm(RVM_PUSH, R0, XX, XX, 0));
...
//	rvm_codegen_addins(co->cg, rvm_asm(RVM_LDRR, R1, R1, XX, 0)); 	// Array
//	rvm_codegen_addins(co->cg, rvm_asm(RVM_MOV, R2, R0, XX, 0)); 	// Offset
...
int codegen_identifier_to_ptr_callback(const char *name, void *userdata, const char *input, unsigned int size, unsigned int reason, const char *start, const char *end)
{
	rvm_compiler_t *co = (rvm_compiler_t *)userdata;
	rulong off = rvm_codegen_getcodesize(co->cg);
	rvm_varmap_t *v = rvm_scope_lookup(co->scope, input, size);
...
void rpagen_load_rules(rpa_dbex_handle dbex, rvm_compiler_t *co)
{
	...
	rpa_dbex_add_callback_exact(dbex, "NegativeExpressionOp", RPA_REASON_MATCHED, codegen_unary_asmop_callback, co);
